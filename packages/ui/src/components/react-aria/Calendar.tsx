...
const cellStyles = tv({
  ...
  variants: {
    isToday: {
      true: 'font-semibold bg-gray-600/5',
    },
    isSelected: {
      false: 'text-gray-700 hover:bg-gray-600/10',
...
export function CalendarHeader() {
  return (
    <header className="flex items-center gap-1 pl-2 pb-1">
      <Heading className="flex-1 my-0 text-gray-600 text-sm font-semibold capitalize" />
      <Button
        variant="icon"
        slot="previous"
        className="rounded-full w-8 h-8 text-blue-600 focus:outline-none data-[focus-visible=true]:ring-2"
      >
...
export function CalendarGridHeader() {
  return (
    <AriaCalendarGridHeader>
      {(day) => (
        <CalendarHeaderCell className="text-xs text-gray-400 font-semibold text-center leading-8 py-0">
          {day}
        </CalendarHeaderCell>
      )}
    </AriaCalendarGridHeader>
