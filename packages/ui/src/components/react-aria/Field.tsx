...
export const fieldBorderStyles = tv({
  variants: {
    isFocusWithin: {
      ...
      true: 'outline-none ring-primary/15 ring-3 border-primary',
    },
    isInvalid: {
...
export const fieldGroupStyles = tv({
  base: 'group flex h-9 w-full rounded-md border transition-[box-shadow] bg-transparent text-sm shadow-sm cursor-text bg-white',
  ...
  return (
    <RACInput
      ...
      className={composeTailwindRenderProps(
        props.className,
        'px-2.5 py-1.5 flex-1 min-w-0 outline-0 bg-white dark:bg-gray-900 text-sm text-gray-800 dark:text-gray-200 disabled:text-gray-200 dark:disabled:text-gray-600',
      )}
    />
  )
}
